    ...
    <section className="mt-16 md:mt-24">
      ...
      <Tabs defaultValue={tabs[0].value}>
        <ScrollArea className="w-full whitespace-nowrap">
          <TabsList className="inline-flex h-auto p-1.5 bg-card/50 justify-start w-auto mx-auto mb-8 rounded-full">
            {tabs.map(tab => (
              <TabsTrigger 
                key={tab.value} 
                value={tab.value}
                className={cn(
                  ...
                  "data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md data-[state=inactive]:bg-transparent data-[state=inactive]:text-muted-foreground data-[state=inactive]:hover:bg-muted/80"
                  )}
              >
                {tab.label}
              </TabsTrigger>
